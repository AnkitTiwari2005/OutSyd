    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between pb-2 mb-4 border-b border-[var(--border-color)]">
          ...
          {bua > 0 && (
            <span className="text-xs font-semibold px-2.5 py-1 rounded bg-[var(--accent-navy-subtle)] text-[var(--accent-navy)] border border-[var(--border-muted)]">
                  ...
                  {footprint > 0 && (
                    <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[11px]">
                      <span className="text-[var(--text-muted)]">
                        Footprint: <strong className="text-[var(--accent-navy)]">{footprint.toLocaleString('en-IN')} sqft</strong>
                      </span>
                      {plotArea > 0 && (
                      ...
                      {coveragePct > 85 && (
                        <button
                          ...
                          className="text-[11px] text-[var(--accent-navy)] font-semibold underline hover:text-[var(--accent-cta)] cursor-pointer ml-auto"
                        >
      ...
      <div>
        <div className="pb-2 mb-4 border-b border-[var(--border-color)]">
                ...
                return (
                  <button
                    ...
                    className={`relative flex flex-col items-start p-3.5 rounded-lg text-left transition-all cursor-pointer ${
                      selected
                        ? 'border-2 border-[var(--accent-navy)] bg-[var(--accent-navy-subtle)]'
                        : 'border border-[var(--border-color)] bg-[var(--bg-card)] hover:border-[var(--border-muted)]'
                    }`}
                  >
                    ...
                    <Icon size={18} className={selected ? 'text-[var(--accent-navy)]' : 'text-[var(--text-muted)]'} />
                    <span className="font-bold text-sm text-[var(--text-primary)] mt-2">{label}</span>
                  </button>
                );
              })}
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div>
            <label className="text-xs font-semibold text-[var(--text-primary)] block mb-2">Soil Bearing Condition</label>
            <Controller
              control={control}
              name="soilType"
              render={({ field }) => (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 mb-1">
                          <Icon size={14} className={selected ? 'text-[var(--accent-navy)]' : 'text-[var(--text-muted)]'} />
          ...
          <div>
            ...
            >
              ...
              {locationRegion && (() => {
                ...
                if (matchedCity) {
                  return (
                    <div className="mt-2 p-2 rounded bg-[var(--success-bg)] border border-[var(--success-border)] flex items-center gap-1.5 text-xs text-[var(--success-text)] font-semibold">
                ...
                if (locationRegion.trim().length >= 3) {
                  return (
                    <p className="text-xs text-[var(--warning-text)] mt-1.5">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      <Icon size={16} className={selected ? 'text-[var(--accent-navy)]' : 'text-[var(--text-muted)]'} />
                      <span className="font-bold text-sm text-[var(--text-primary)]">{label}</span>
                    </div>
                    {/* Dynamic regional index price range (U-8) */}
                    <span className="text-xs font-mono font-bold text-[var(--accent-navy)] tabular-nums mt-0.5">
                      {rangeDisplay}
                    </span>
                    <p className="text-xs text-[var(--text-muted)] mt-2 leading-relaxed">{desc}</p>
                    <div className="mt-3 pt-2 border-t border-[var(--border-color)] text-[11px] text-[var(--text-primary)]">
                      <span className="font-semibold text-[var(--text-muted)]">Best for:</span> {bestFor}
                    </div>
                  </button>
                );
              })}
